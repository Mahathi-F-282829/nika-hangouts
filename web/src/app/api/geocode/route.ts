...
/**
 * Compute a loose bbox for a single GeoJSON feature.
 * Note: points yield a zero-area bbox; used for quick extent hints.
 */
function bboxOfFeature(f: GeoJSON.Feature): [number, number, number, number] | null {
    const g = f.geometry;
    if (!g) return null;
    ...
    const walk = (coords: any) => {
        if (typeof coords[0] === "number") extend(coords[0], coords[1]);
    ...
    switch (g.type) {
        ...
        case "GeometryCollection":
            (g as GeoJSON.GeometryCollection).geometries.forEach((sub) => {
                const tmp = bboxOfFeature({ type: "Feature", geometry: sub, properties: {} });
                if (tmp) {
                    extend(tmp[0], tmp[1]);
                    extend(tmp[2], tmp[3]);
                }
